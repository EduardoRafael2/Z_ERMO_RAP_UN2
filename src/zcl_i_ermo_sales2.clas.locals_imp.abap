...
CLASS lhc_Sales IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lf_sales>).
      ...
      DATA(lo_instance) = zcl_ermo_sales2=>get_instance( ).
      DATA(ls_sales) = CORRESPONDING ztermo_sales_un2( <lf_sales> ).

      ls_sales-orderid = lo_uuid->create_uuid_x16( ).
      APPEND ls_sales TO zcl_ermo_sales2=>gt_sales.
  ...
  TYPES: tt_sales   TYPE TABLE OF ztermo_sales_un2 WITH DEFAULT KEY,
           tt_sales_x TYPE TABLE OF zsermo_sales_un2 WITH DEFAULT KEY.
    ...
    IF lt_sales IS NOT INITIAL.
      ...
      zcl_ermo_sales2=>gt_sales = VALUE #( FOR ls IN lt_sales
...
CLASS lsc_ZI_ERMO_SALES2 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_ERMO_SALES2 IMPLEMENTATION.
